...
const Users: React.FC = () => {
    ...
    const loadUsers = useCallback(async () => {
        ...
        try {
            ...
            const response = await userService.getUsersWithFilters(filterParams, page, 20);
            clearLoadingTimeout();
    ...
    }, [page, searchTerm, filters.role, filters.status]);
    ...
    }, [page, searchTerm, filters.role, filters.status, loadUsers]);
    ...
    const EmptySkeletonCard = () => (
        <div className="bg-white rounded-2xl shadow-md overflow-hidden">
            ...
            <div className="p-4 pt-6 text-center">
                ...
                <div className="h-4 bg-gray-200 rounded w-56 mx-auto mb-4"></div>
                <div className="h-10 bg-gray-200 rounded w-32 mx-auto"></div>
            </div>
        </div>
    );

    const EmptySearchSkeletonCard = () => (
        <div className="bg-white rounded-2xl shadow-md overflow-hidden">
            <div className="h-24" style={{ background: `linear-gradient(135deg, ${COLORS.primary} 0%, #2A5C8E 100%)` }} />
            <div className="relative flex justify-center -mt-12">
                <div className="w-24 h-24 rounded-full flex items-center justify-center bg-gray-100 border-4 border-white shadow-lg">
    ...
    const ErrorState = () => (
        <div className="flex flex-col items-center justify-center h-[60vh]">
            <div className="bg-white rounded-xl p-8 text-center max-w-md" style={{ backgroundColor: COLORS.white }}>
            ...
            <div className="p-4 md:p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-white rounded-xl shadow-md p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm" style={{ color: COLORS.text, opacity: 0.7 }}>Utilisateurs actifs</p>
                                <p className="text-2xl font-bold mt-1" style={{ color: COLORS.warning }}>{stats?.activeUsers ?? 0}</p>
                            </div>
                            <div className="p-3 rounded-full" style={{ backgroundColor: 'rgba(255, 193, 7, 0.15)' }}>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold mt-1" style={{ color: COLORS.primary, opacity: 0.7 }}>{stats?.lockedUsers ?? 0}</p>
                            </div>
                            <div className="p-3 rounded-full" style={{ backgroundColor: COLORS.borderLight }}>
                                <ShieldAlert className="w-5 h-5" style={{ color: COLORS.primary, opacity: 0.6 }} />
                            </div>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {searchTerm ? <EmptySearchSkeletonCard /> : <EmptySkeletonCard />}
                    </div>
                ) : viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {users.map((user) => (
                            <UserCard key={user.id} user={user} onEdit={handleEdit} onDelete={handleDelete} onLock={handleLock} onUnlock={handleUnlock} />
                        ))}
                    </div>
                ) : (
                            ...
                            <tbody>
                                {users.map((user) => (
                                    <tr key={user.id} className="border-b hover:bg-gray-50 transition-colors" style={{ borderColor: COLORS.border }}>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                {user.imageUrl ? (
                                                    ...
                                                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-medium" style={{ backgroundColor: COLORS.primary }}>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className="px-2 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: COLORS.borderLight, color: COLORS.primary }}>
