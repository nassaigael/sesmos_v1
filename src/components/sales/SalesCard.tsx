    ...
    ({ onAdd, searchTerm, hasFilters, onClearFilters }) => {
        ...
        return (
            <div className="bg-white rounded-2xl shadow-md overflow-hidden">
                ...
                <div className="relative flex justify-center -mt-12">
                    <div className="w-24 h-24 rounded-full flex items-center justify-center bg-gray-100 border-4 border-white shadow-lg">
                        <DollarSign className="w-10 h-10" style={{ color: COLORS.primary, opacity: 0.3 }} />
                    </div>
                </div>
                <div className="p-6 pt-6 text-center">
                    <h3 className="text-xl font-bold mb-2" style={{ color: COLORS.primary }}>Aucune vente</h3>
...
const SalesList: React.FC<SalesListProps> = ({
    ...
    const [filters, setFilters] = useState({
        ...
        productId: externalFilters?.productId || '',
        regionId: externalFilters?.regionId || '',
        userId: externalFilters?.userId || '',
        minAmount: externalFilters?.minAmount || '',
        maxAmount: externalFilters?.maxAmount || ''
    });
    ...
    }, [filters, onFilterCountChange, onFilterChange]);
    ...
    const loadSales = useCallback(async (resetPage?: boolean) => {
        ...
        try {
            ...
            if (filters.productId) filterParams.productId = filters.productId;
            if (filters.regionId) filterParams.regionId = filters.regionId;
            ...
            const response = await saleService.getSalesWithFilters(filterParams, currentPage, 12);
            setSales(response.content || []);
            setTotalPages(response.totalPages || 0);
            setTotalElements(response.totalElements || 0);
            onTotalCountChange?.(response.totalElements || 0);
    ...
    // Rechargement quand les filtres ou la recherche changent
    useEffect(() => {
        if (initialLoadDone.current) {
            loadSales(true);
        }
    }, [filters, searchTerm, loadSales]);
    ...
    // Afficher le skeleton seulement si loading ET pas de données
    if (loading && sales.length === 0) {
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => (
    ...
    // Indicateur de chargement léger quand on recharge mais qu'on a déjà des données
    const showReloading = loading && sales.length > 0;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Indicateur de rechargement léger */}
            {showReloading && (
                <div className="flex items-center justify-center gap-2 py-2">
                    <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: COLORS.accent }} />
                    <span className="text-xs" style={{ color: COLORS.primary, opacity: 0.6 }}>Mise à jour...</span>
                </div>
            )}
            ...
            {showForm && (
                <SaleForm
                    isOpen={showForm}
                    onClose={() => { setShowForm(false); setSelectedSale(null); }}
                    onSuccess={() => { loadSales(true); onRefresh?.(); }}
                    editSale={selectedSale || undefined}
                />
            )}
