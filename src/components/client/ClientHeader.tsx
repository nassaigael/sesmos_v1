...
const ClientHeader: React.FC<ClientHeaderProps> = ({ toggleSidebar, isMobile }) => {
			...
			<div className="px-4 md:px-6 py-4">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-4">
						{isMobile && (
							<button onClick={toggleSidebar} className="p-2 rounded-lg transition-all duration-200 hover:bg-gray-100">
					...
					<div className="flex items-center gap-2 md:gap-4">
						<div className="relative" ref={notificationRef}>
							...
							>
								...
								{unreadCount > 0 && (
									<span className="absolute -top-1 -right-1 min-w-5 h-5 rounded-full text-xs flex items-center justify-center px-1 animate-pulse"
							...
							<button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 md:gap-3 p-1 md:p-2 rounded-lg transition-all duration-200 hover:bg-gray-100 group">
								<div className="relative">
									{clientData?.logoUrl && !imageError ? (
										<img src={clientData.logoUrl} alt={clientData.companyName || 'Avatar'} className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover border-2 transition-all group-hover:scale-105" style={{ borderColor: COLORS.accent }} onError={() => setImageError(true)} />
									) : (
										<div className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm md:text-base transition-all group-hover:scale-105" style={{ backgroundColor: COLORS.accent }}>
								...
								<div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl overflow-hidden z-50 border animate-fadeInUp" style={{ borderColor: COLORS.border }}>
									<div className="p-4 border-b" style={{ borderColor: COLORS.border }}>
										<div className="flex items-center gap-3">
											{clientData?.logoUrl && !imageError ? (
												<img src={clientData.logoUrl} alt={clientData.companyName || 'Avatar'} className="w-12 h-12 rounded-full object-cover border-2" style={{ borderColor: COLORS.accent }} />
											) : (
												<div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: COLORS.accent }}>
													{clientData?.companyName ? getInitials(clientData.companyName) : 'C'}
												</div>
											)}
											<div className="flex-1">
												...
												<span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-xs" style={{ backgroundColor: `${COLORS.accent}15`, color: COLORS.accent }}>
									...
									<div className="py-2">
										<button onClick={() => { window.location.href = '/client/profile'; setIsProfileOpen(false); }} className="w-full px-4 py-2.5 text-left transition-colors hover:bg-gray-50 flex items-center gap-3 group">
										...
										<button onClick={() => { window.location.href = '/client/company'; setIsProfileOpen(false); }} className="w-full px-4 py-2.5 text-left transition-colors hover:bg-gray-50 flex items-center gap-3 group">
											<div className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors group-hover:bg-yellow-50" style={{ backgroundColor: `${COLORS.accent}10` }}>
										...
										<button onClick={() => { window.location.href = '/client/chat'; setIsProfileOpen(false); }} className="w-full px-4 py-2.5 text-left transition-colors hover:bg-gray-50 flex items-center gap-3 group">
											<div className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors group-hover:bg-yellow-50" style={{ backgroundColor: `${COLORS.accent}10` }}>
										...
										<button onClick={() => { window.location.href = '/client/settings'; setIsProfileOpen(false); }} className="w-full px-4 py-2.5 text-left transition-colors hover:bg-gray-50 flex items-center gap-3 group">
											<div className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors group-hover:bg-yellow-50" style={{ backgroundColor: `${COLORS.accent}10` }}>
