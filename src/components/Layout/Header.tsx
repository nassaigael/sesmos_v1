                ...
                <div className="px-4 md:px-6 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 md:gap-4">
                            {showAddButton && onAdd && (
                                <button onClick={onAdd} className="flex items-center gap-2 px-3 py-2 rounded-lg text-white transition-colors hover:opacity-90" style={{ backgroundColor: COLORS.primary }}>
                            ...
                            {showViewToggle && currentPage === 'sales' && onViewModeToggle && (
                                <button onClick={onViewModeToggle} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-300 hover:bg-gray-100" style={{ backgroundColor: 'rgba(26, 60, 94, 0.05)' }}>
                                    <span className={`transition-all duration-300 ${viewMode === 'list' ? 'text-[#FFC107] font-semibold' : 'text-gray-500 hover:text-gray-700'}`}>Ventes</span>
                                    <span className="text-gray-300">|</span>
                                    <span className={`transition-all duration-300 ${viewMode === 'analytics' ? 'text-[#FFC107] font-semibold' : 'text-gray-500 hover:text-gray-700'}`}>Analyses</span>
                                </button>
                            )}

                            {showViewToggle && currentPage !== 'sales' && onViewModeToggle && (
                                <button onClick={onViewModeToggle} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'grid' ? 'text-white shadow-sm' : 'hover:bg-gray-100'}`} style={viewMode === 'grid' ? { backgroundColor: COLORS.primary } : { color: COLORS.primary }}>
                                ...
                                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 md:gap-3 p-1 md:p-2 rounded-lg transition-all duration-200 hover:bg-gray-100 group">
                                    <div className="relative">
                                        {user?.imageUrl && !imageError ? (
                                            <img src={user.imageUrl} alt={user.name || 'Avatar'} className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover border-2 transition-all group-hover:scale-105" style={{ borderColor: COLORS.accent }} onError={() => setImageError(true)} />
                                        ) : (
                                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm md:text-base transition-all group-hover:scale-105" style={{ backgroundColor: getRoleColor(user?.role) }}>
                                    ...
                                    <div className="hidden md:block text-left">
                                        <p className="text-sm font-medium" style={{ color: COLORS.primary }}>{user?.name || 'Utilisateur'}</p>
                    ...
                    {(showSearch || showFilter) && (
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mt-4 pt-2 border-t" style={{ borderColor: COLORS.border }}>
                            {showSearch && (
                                <div className="flex-1 w-full sm:max-w-md relative">
                                    ...
                                    <input type="text" placeholder="Rechercher..." value={localSearchTerm} onChange={(e) => handleSearchChange(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-lg border focus:outline-none focus:ring-2 transition-all" style={{ borderColor: COLORS.border }} onFocus={(e) => e.target.style.borderColor = COLORS.accent} onBlur={(e) => e.target.style.borderColor = COLORS.border} />
                                    {localSearchTerm && (
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                {showFilter && onFilterToggle && (
                                    <button onClick={onFilterToggle} className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-all ${filterActive ? 'text-white' : 'hover:bg-gray-50'}`} style={filterActive ? { backgroundColor: COLORS.primary, borderColor: COLORS.primary } : { borderColor: COLORS.border }}>
